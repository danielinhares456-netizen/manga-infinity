...
export default function DetailsView({ manga, libraryData, historyData, user, userProfileData, onBack, onChapterClick, onRequireLogin, showToast }) {
  ...
  // TRAVA 1: Se a obra piscar vazia, não quebra a tela
                 ...
                 <div className="space-y-2">
                    {(!manga.chapters || manga.chapters.length === 0) ? (
                       ...
                       manga.chapters.map(cap => {
                           // TRAVA 2: Ignora o capítulo fantasma que fez a tela quebrar
                           if (!cap) return null; 
                           
                           const isRead = historyData.some(h => h.mangaId === manga.id && h.chapterNumber === cap.number);
                           return (
                               <div key={cap.id || Math.random()} onClick={() => onChapterClick(manga, cap)} className={`flex justify-between items-center p-4 rounded-xl border cursor-pointer transition-colors duration-300 ${isRead ? 'bg-[#050508]/50 border-white/5' : 'bg-[#0d0d12] border-white/10 hover:border-cyan-500/50'}`}>
