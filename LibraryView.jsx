...
export function LibraryView({ mangas, user, libraryData, onNavigate, onRequireLogin, dataSaver }) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 animate-in fade-in duration-300">
      <div className="flex items-center gap-3 mb-6"><Library className="w-6 h-6 text-fuchsia-400" /><h2 className="text-2xl font-black text-white">Minha Biblioteca</h2></div>
      ...
      {libraryMangas.length === 0 ? (
         <div className="text-center py-16 bg-[#0d0d12]/50 rounded-xl border border-white/10 border-dashed"><BookOpen className="w-12 h-12 mx-auto text-gray-400/30 mb-3"/><p className="text-gray-400/60 font-bold text-sm">Nenhuma obra marcada como "{activeStatus}".</p></div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 md:gap-4">
          {libraryMangas.map(manga => (
            <div key={manga.id} className="cursor-pointer group flex flex-col gap-1.5" onClick={() => onNavigate('details', manga)}>
              <div className={`relative aspect-[2/3] rounded-lg overflow-hidden bg-[#0d0d12] border border-white/10 shadow-sm ${dataSaver ? 'blur-[1px]' : ''}`}><img src={manga.coverUrl} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" /></div>
