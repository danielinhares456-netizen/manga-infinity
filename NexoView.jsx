...
export function NexoView({ user, userProfileData, showToast, mangas, onNavigate, onLevelUp, synthesizeCrystal, shopItems, buyItem }) {
  ...
  const generateMission = async (difficulty) => { 
    try { 
      ...
      showToast("Falha na Matrix do Nexo.", "error"); 
    } finally { 
      setIsForgingMission(false); 
    } 
  }; 
  ...
  const handleEnigmaSubmit = async (e) => { 
    ...
    if (isCorrect) { 
      ...
      setEnigmaAnswer(''); 
      showToast(`Decifrado! +${m.rewardXp} XP / +${m.rewardCoins} M.`, "success"); 
      ...
      if (attempts <= 0) { 
        ...
        showToast(`Incorreto. ${attempts} vida(s) restante(s).`, "error"); 
      } 
      setEnigmaAnswer(''); 
    } 
  }; 
  ...
  // FUNÇÃO DE INJEÇÃO CONSOLIDADA PARA EVITAR CONFLITOS CSS
  const renderDynamicStyles = (eqObj) => {
    if (!eqObj) return null;
    const dynamicStyles = Object.values(eqObj)
      .filter(Boolean)
      .map(item => `
        .${item.cssClass} {
          ${item.css || ''}
  ...
  return ( 
    <div className={`max-w-4xl mx-auto px-4 py-6 animate-in fade-in duration-500 relative pb-24 font-sans text-gray-300 min-h-screen ${equipped.tema_perfil ? equipped.tema_perfil.cssClass : 'bg-[#020202]'}`}> 
      ...
      {/* INJEÇÃO DO TEMA DO USUÁRIO LOGADO (Global da Tela) */} 
      {renderDynamicStyles(equipped)}
                    ...
                    return ( 
                      <div className="flex flex-col items-center animate-in slide-in-from-bottom-10 duration-700 delay-100 group relative"> 
                        ...
                        <div className="relative w-20 h-20 sm:w-24 sm:h-24 flex items-center justify-center transition-transform duration-500 group-hover:scale-110"> 
                          {eq.particulas && <img src={eq.particulas.preview} className={`absolute inset-[-50%] w-[200%] h-[200%] max-w-none pointer-events-none z-0 ${eq.particulas.cssClass}`} />} 
                          ...
                          {eq.efeito && <img src={eq.efeito.preview} className={`absolute inset-0 w-full h-full pointer-events-none z-20 ${eq.efeito.cssClass}`} />} 
