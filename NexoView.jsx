        ...
        <div className={`max-w-4xl mx-auto px-4 py-6 animate-in fade-in duration-500 relative pb-24 font-sans text-gray-300 min-h-screen ${equipped.tema_perfil ? equipped.tema_perfil.cssClass : 'bg-[#020205]'}`}>
            ...
            {isForgingMission && (
                <div className="fixed inset-0 z-[3000] bg-black/95 backdrop-blur-xl flex flex-col items-center justify-center animate-in fade-in duration-300 w-full">
                    ...
                    <div className="relative w-48 h-48 flex items-center justify-center"><div className="absolute inset-0 bg-gradient-to-tr from-blue-600 via-black to-red-600 rounded-full blur-[45px] animate-pulse opacity-60"></div><div className="absolute inset-4 border-[2px] border-blue-900/40 border-dashed rounded-full animate-[spin_4s_linear_infinite]"></div><div className="absolute inset-8 border-[3px] border-t-blue-500 border-b-red-600 border-l-transparent border-r-transparent rounded-full animate-[spin_1.5s_linear_infinite]"></div><div className="absolute inset-12 bg-[#020205]/80 backdrop-blur-md rounded-full shadow-[0_0_30px_rgba(245,158,11,0.15)] flex items-center justify-center"><Zap className="w-10 h-10 text-amber-400 drop-shadow-[0_0_15px_rgba(245,158,11,0.8)] animate-pulse" /></div></div>
                    <h2 className="mt-12 text-lg md:text-xl font-black text-center text-transparent bg-clip-text bg-gradient-to-r from-blue-100 via-white to-amber-500 tracking-[0.5em] md:tracking-[0.6em] animate-pulse uppercase">Forjando Contrato...</h2>
                </div>
            )}
            {confirmModal && (
            ...
            {activeTab === "Missões" && (
                <div className="animate-in fade-in duration-300">
                    {userProfileData.activeMission ? (
                        <div className="bg-[#050508] border border-blue-900/30 p-5 sm:p-6 rounded-3xl shadow-[0_0_50px_rgba(37,99,235,0.05)] mb-6 animate-in zoom-in-95 overflow-hidden relative">
                            <div className="absolute top-0 left-0 w-full h-[2px] bg-gradient-to-r from-blue-800 via-amber-500 to-red-600"></div>
                            <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                ...
                                <span className={`text-[9px] font-black px-3 py-1.5 rounded-full uppercase tracking-[0.2em] border shadow-sm w-max ${userProfileData.activeMission.difficulty.includes('S') ? 'bg-red-950/50 text-red-500 border-red-500/50' : 'bg-amber-950/50 text-amber-400 border-amber-500/50'}`}>
                                    {userProfileData.activeMission.difficulty}
                                </span>
                            </div>
                            <div className="mb-6">
                                {userProfileData.activeMission.type === 'search_local' ? (
                                    <div className="bg-black p-5 rounded-2xl border border-blue-900/20 relative shadow-inner">
                                        <div className="flex items-center gap-2.5 mb-4 text-blue-500"><Compass className="w-5 h-5"/><span className="font-black text-[10px] uppercase tracking-widest">Caçada Visual</span></div>
                                        <p className="text-xs font-medium text-gray-300 mb-5 leading-relaxed whitespace-pre-wrap border-l-2 border-blue-600/50 pl-4">{userProfileData.activeMission.question}</p>
                                    </div>
                                    ...
                                    <div className="bg-black p-5 rounded-2xl border border-blue-900/20 relative shadow-inner">
                                        ...
                                        <form onSubmit={handleEnigmaSubmit} className="flex flex-col sm:flex-row gap-3 relative z-10">
                                            <div className="relative flex-1"><Key className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-amber-500" /><input type="text" value={enigmaAnswer} onChange={e=>setEnigmaAnswer(e.target.value)} placeholder="Digite o eco exato da obra..." className="w-full bg-[#020205] border border-blue-900/30 rounded-xl pl-11 pr-4 py-3.5 text-white outline-none focus:border-amber-600/50 transition-colors duration-300 font-bold text-xs shadow-inner" /></div>
                                            <div className="flex gap-2">
                                                <button type="submit" className="bg-gradient-to-r from-blue-800 to-amber-600 text-white font-black px-6 py-3.5 rounded-xl hover:scale-[1.02] transition-transform flex items-center justify-center gap-2 text-[10px] uppercase tracking-widest shadow-md duration-300">Decifrar <Check className="w-4 h-4"/></button>
                                                <div className="bg-[#020205] px-4 py-3.5 rounded-xl border border-blue-900/30 text-[10px] font-black text-gray-500 uppercase tracking-widest flex items-center justify-center gap-2 shadow-inner">Vidas: <span className={userProfileData.activeMission.attemptsLeft === 1 ? 'text-red-500' : 'text-white'}>{userProfileData.activeMission.attemptsLeft}</span></div>
                                            </div>
                                        </form>
                                    </div>
                                ) : (
                                    <div className="bg-black p-5 rounded-2xl border border-blue-900/20 shadow-inner">
                                        <div className="flex justify-between items-end mb-3"><span className="text-[10px] font-black text-blue-900/60 uppercase tracking-widest flex items-center gap-2"><Target className="w-4 h-4 text-blue-600"/> Progresso do Contrato</span><span className="text-xl font-black text-white tracking-tighter">{userProfileData.activeMission.currentCount} <span className="text-gray-600 text-xs">/ {userProfileData.activeMission.targetCount}</span></span></div>
                                        <div className="w-full bg-[#020205] rounded-full h-2 overflow-hidden border border-blue-900/30 shadow-inner"><div className="bg-gradient-to-r from-blue-800 to-amber-500 h-full rounded-full transition-all duration-500 relative" style={{width: `${(userProfileData.activeMission.currentCount / userProfileData.activeMission.targetCount) * 100}%`}}><div className='absolute inset-0 bg-white/20 animate-pulse'></div></div></div>
                                        <p className="mt-4 text-[11px] text-blue-200/60 font-medium text-center italic">{userProfileData.activeMission.desc}</p>
                                    </div>
                                )}
                            </div>
                            <div className="bg-black p-5 rounded-2xl border border-blue-900/20 shadow-inner">
                                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                                    <div className="flex flex-wrap items-center gap-2.5">
                                        <span className="text-[10px] text-blue-900/60 font-black uppercase tracking-widest mr-1">Recompensas:</span>
                                        <span className="text-xs font-black text-white bg-blue-900/20 px-3 py-1.5 rounded-lg border border-blue-900/30 shadow">+{userProfileData.activeMission.rewardXp} XP</span>
                                    ...
                                    <div className="flex w-max mx-auto sm:mx-0 items-center gap-2 text-blue-500 bg-blue-950/20 px-4 py-2 rounded-full border border-blue-900/30 shadow"><Timer className="w-4 h-4 animate-pulse"/><span className="font-black text-xs tracking-wider">{timeLeft}</span></div>
                                </div>
                                <div className="w-full h-[1px] bg-blue-900/20 my-3"></div>
                                <div className="flex flex-col sm:flex-row gap-4 sm:gap-0 sm:items-center justify-between">
                                    <span className="text-[10px] text-blue-900/80 font-bold uppercase tracking-wider text-center sm:text-left">Punição: -{userProfileData.activeMission.penaltyXp}XP / -{userProfileData.activeMission.penaltyCoins} M</span>
                        ...
                        <div className="flex flex-col gap-3">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-1">
                                {RANK_CARDS.map(rank => {
                                    ...
                                    return (
                                    <div key={rank.id} className={`bg-[#050508] border ${rank.border} ${rank.hover} transition-all duration-300 p-6 rounded-3xl flex flex-col group shadow-md hover:shadow-[0_0_30px_rgba(37,99,235,0.05)]`}>
                                        ...
                                        <div className="flex flex-col gap-2 mb-6 mt-2 bg-black p-4 rounded-2xl border border-blue-900/20 shadow-inner">
                                            <div className="flex justify-between text-xs text-gray-500"><span className="flex items-center gap-2 font-black tracking-widest uppercase text-[9px]"><Target className="w-3.5 h-3.5 text-blue-900/60"/> Mistérios Ocultos</span><span className="font-bold text-gray-300 text-[10px] uppercase">Aleatório</span></div>
                                            <div className="flex justify-between text-xs text-gray-500"><span className="flex items-center gap-2 font-black tracking-widest uppercase text-[9px]"><Clock className="w-3.5 h-3.5 text-blue-900/60"/> Tempo Est.</span><span className="font-bold text-gray-300 text-[10px] uppercase">~ {conf.time} min</span></div>
                                        </div>
            ...
            {activeTab === "Forja" && (
                <div className="animate-in fade-in duration-300">
                    <div className="bg-gradient-to-br from-[#050508] to-black border border-blue-900/20 p-6 md:p-8 rounded-3xl shadow-2xl relative overflow-hidden">
                        <div className="absolute inset-0 bg-gradient-to-b from-blue-950/10 via-transparent to-transparent opacity-50"></div>
                       <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-8 relative z-10">
                         ...
                         <div className="flex gap-3">
                            <div className="bg-black border border-blue-900/20 p-5 rounded-2xl text-center min-w-[100px] shadow-inner"><p className="text-3xl font-black text-blue-500 tracking-tighter">{userProfileData.crystals || 0}</p><p className="text-[9px] text-gray-600 uppercase font-black tracking-widest mt-1">Cristais</p></div>
                            <div className="bg-black border border-blue-900/20 p-5 rounded-2xl text-center min-w-[100px] shadow-inner"><p className="text-3xl font-black text-amber-500 tracking-tighter">{userProfileData.coins || 0}</p><p className="text-[9px] text-gray-600 uppercase font-black tracking-widest mt-1">Moedas M</p></div>
                         </div>
                       </div>
                       <div className="bg-black border border-blue-900/20 p-8 rounded-3xl relative overflow-hidden flex flex-col items-center justify-center text-center shadow-inner group">
                          <div className={`w-28 h-28 rounded-full flex items-center justify-center mb-6 relative transition-all duration-1000 ${synthesizing ? 'scale-110' : ''}`}><div className={`absolute inset-0 rounded-full border-2 border-t-blue-600 border-r-amber-600 border-b-transparent border-l-transparent ${synthesizing ? 'animate-[spin_0.5s_linear_infinite] opacity-100' : 'opacity-20'}`}></div><div className={`absolute inset-2 rounded-full border border-blue-600/20 ${synthesizing ? 'opacity-100' : 'opacity-40'}`}></div><Zap className={`w-12 h-12 ${synthesizing ? 'text-white animate-pulse drop-shadow-[0_0_20px_rgba(245,158,11,0.8)]' : 'text-amber-500'}`} strokeWidth={1.5}/></div>
                          <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mb-6">Custo: 5 Cristais (40% de chance de falha).</p>
                          <button onClick={runSynthesis} disabled={synthesizing || (userProfileData.crystals || 0) < 5} className="w-full sm:w-64 bg-gradient-to-r from-blue-800 to-amber-600 text-white font-black py-4 rounded-xl flex items-center justify-center gap-2 hover:scale-105 transition-transform disabled:from-gray-900 disabled:to-gray-900 disabled:opacity-50 disabled:hover:scale-100 text-[10px] uppercase tracking-[0.2em] shadow-[0_0_15px_rgba(245,158,11,0.3)] duration-300">{synthesizing ? 'SINTETIZANDO...' : 'SINTETIZAR (-5)'}</button>
                       </div>
                    </div>
                </div>
            )}
            ...
            {activeTab === "Ranking" && (
                <div className="animate-in fade-in duration-500">
                    <div className="bg-gradient-to-b from-[#050508] to-black border border-blue-900/20 p-6 sm:p-8 rounded-3xl shadow-2xl relative overflow-hidden">
                        ...
                        <div className="absolute bottom-0 left-0 w-64 h-64 bg-blue-800/5 blur-[80px] rounded-full pointer-events-none"></div>
                        ...
                        <div className="text-center mb-12 relative z-10 flex flex-col items-center justify-center">
                            ...
                            <p className="text-blue-900/80 text-[10px] sm:text-xs font-black uppercase tracking-widest text-center">As Entidades supremas do Abismo Cósmico.</p>
                        </div>
                        ...
                        {loadingRank ? (
                            ...
                            <div className="py-16 text-center border border-dashed border-blue-900/30 rounded-3xl bg-black/50 shadow-inner animate-in fade-in">
                                <ShieldAlert className="w-12 h-12 text-blue-900/40 mx-auto mb-4" />
                            ...
                            <div className="relative z-10 animate-in fade-in duration-700">
                                <div className="flex justify-center items-end gap-3 sm:gap-6 mb-16 mt-6 px-2">
                                    {rankingList[1] && (() => {
                                        ...
                                        // CSS INJECTION PARA ESSE USUARIO NO RANKING
                                        const dStyle = Object.values(eq).filter(Boolean).map(i => `.${i.cssClass} { ${i.css || ''} } ${i.animacao || ''}`).join('\n');
                                        return (
                                        <div className="flex flex-col items-center animate-in slide-in-from-bottom-10 duration-700 delay-100 group relative">
                                            {dStyle && <style dangerouslySetInnerHTML={{__html: dStyle}} />}
                                            ...
                                            <div className="relative w-20 h-20 sm:w-24 sm:h-24 flex items-center justify-center transition-transform duration-500 group-hover:scale-110">
                                                ...
                                                <div className={`w-full h-full rounded-full border-[5px] border-gray-400 bg-black shadow-[0_0_30px_rgba(156,163,175,0.3)] overflow-hidden relative z-10`}>
                                                    <img src={eq.avatar ? eq.avatar.preview : (p.avatarUrl || `https://placehold.co/100x100/020205/a1a1aa?text=2`)} className={`w-full h-full object-cover ${eq.avatar?.cssClass || ''}`} />
                                                </div>
                                            ...
                                            <div className="h-20 sm:h-28 w-20 sm:w-24 bg-gradient-to-t from-black to-gray-800/20 mt-4 rounded-t-2xl border-t-[3px] border-gray-400 flex flex-col items-center justify-start pt-4 shadow-[0_-5px_15px_rgba(0,0,0,0.2)]">
                                                ...
                                                <span className="text-[10px] text-blue-500 font-black mt-1.5 uppercase">Lvl {p.level || 1}</span>
                                            </div>
                                        </div>
                                    )})}
                                        ...
                                        <div className="flex flex-col items-center animate-in slide-in-from-bottom-16 duration-700 z-10 group relative">
                                            ...
                                            <div className="relative w-24 h-24 sm:w-32 sm:h-32 flex items-center justify-center transition-transform duration-500 group-hover:scale-110">
                                                ...
                                                <div className={`w-full h-full rounded-full border-[6px] border-yellow-400 bg-black shadow-[0_0_40px_rgba(250,204,21,0.5)] overflow-hidden relative z-10`}>
                                                    <img src={eq.avatar ? eq.avatar.preview : (p.avatarUrl || `https://placehold.co/100x100/020205/facc15?text=1`)} className={`w-full h-full object-cover ${eq.avatar?.cssClass || ''}`} />
                                                </div>
                                                {eq.efeito && <img src={eq.efeito.preview} className={`absolute inset-0 w-full h-full pointer-events-none mix-blend-screen z-20 ${eq.efeito.cssClass}`} />}
                                                {eq.moldura && <img src={eq.moldura.preview} className={`absolute inset-[-15%] w-[130%] h-[130%] max-w-none pointer-events-none z-30 ${eq.moldura.cssClass}`} />}
                                    ...
                                    {rankingList[2] && (() => {
                                        ...
                                        const eq = p.equipped_items || {};
                                        const dStyle = Object.values(eq).filter(Boolean).map(i => `.${i.cssClass} { ${i.css || ''} } ${item.animacao || ''}`).join('\n');
                                        return (
                                        <div className="flex flex-col items-center animate-in slide-in-from-bottom-8 duration-700 delay-200 group relative">
                                            ...
                                            <div className="relative w-16 h-16 sm:w-20 sm:h-20 flex items-center justify-center transition-transform duration-500 group-hover:scale-110">
                                                ...
                                                <div className={`w-full h-full rounded-full border-[5px] border-amber-700 bg-black shadow-[0_0_25px_rgba(180,83,9,0.3)] overflow-hidden relative z-10`}>
                                                    <img src={eq.avatar ? eq.avatar.preview : (p.avatarUrl || `https://placehold.co/100x100/020205/b45309?text=3`)} className={`w-full h-full object-cover ${eq.avatar?.cssClass || ''}`} />
                                                </div>
                                                {eq.efeito && <img src={eq.efeito.preview} className={`absolute inset-0 w-full h-full pointer-events-none mix-blend-screen z-20 ${eq.efeito.cssClass}`} />}
                                                {eq.moldura && <img src={eq.moldura.preview} className={`absolute inset-[-15%] w-[130%] h-[130%] max-w-none pointer-events-none z-30 ${eq.moldura.cssClass}`} />}
                                            ...
                                            <div className="h-16 sm:h-24 w-16 sm:w-20 bg-gradient-to-t from-black to-amber-900/10 mt-4 rounded-t-2xl border-t-[3px] border-amber-700 flex flex-col items-center justify-start pt-3 shadow-[0_-5px_15px_rgba(0,0,0,0.2)]">
                                                ...
                                                <span className="text-[9px] text-red-500 font-black mt-1 uppercase">Lvl {p.level || 1}</span>
                                            </div>
                                        </div>
                                    )})}
                                </div>

                                <div className="space-y-3 mt-6 bg-black/80 p-5 sm:p-6 rounded-3xl border border-blue-900/20 shadow-inner">
                                    {rankingList.slice(3).map((p, index) => {
                                        const eq = p.equipped_items || {};
                                        const dStyle = Object.values(eq).filter(Boolean).map(i => `.${i.cssClass} { ${i.css || ''} } ${i.animacao || ''}`).join('\n');
                                        return (
                                        <div key={p.id} className="flex items-center gap-4 p-4 bg-[#050508] hover:bg-amber-950/20 border border-blue-900/20 hover:border-amber-600/40 rounded-2xl transition-all duration-300 group shadow-md relative">
                                            {dStyle && <style dangerouslySetInnerHTML={{__html: dStyle}} />}
                                            <div className="w-8 text-center text-blue-900/60 font-black text-sm">{index + 4}º</div>
                                            
                                            <div className="relative w-12 h-12 flex items-center justify-center flex-shrink-0">
                                                {eq.particulas && <img src={eq.particulas.preview} className={`absolute inset-[-50%] w-[200%] h-[200%] max-w-none pointer-events-none ${eq.particulas.cssClass}`} />}
                                                <div className={`w-full h-full rounded-full bg-black border border-white/10 overflow-hidden relative z-10`}>
                                                    <img src={eq.avatar ? eq.avatar.preview : (p.avatarUrl || `https://placehold.co/50x50/020205/3b82f6?text=${index+4}`)} className={`w-full h-full object-cover ${eq.avatar?.cssClass || ''}`} />
                                                </div>
                                                {eq.efeito && <img src={eq.efeito.preview} className={`absolute inset-0 w-full h-full pointer-events-none mix-blend-screen z-20 ${eq.efeito.cssClass}`} />}
                                                {eq.moldura && <img src={eq.moldura.preview} className={`absolute inset-[-15%] w-[130%] h-[130%] max-w-none pointer-events-none z-30 ${eq.moldura.cssClass}`} />}
                                            ...
                                            <div className="flex-1 min-w-0 z-10 relative">
                                                <h4 className={`font-bold text-sm truncate transition-colors ${eq.nickname ? eq.nickname.cssClass : 'text-white group-hover:text-amber-400'}`}>{p.name || 'Explorador'}</h4>
                                            ...
                                            <div className="text-right z-10 relative">
                                                <div className="font-black text-blue-500 text-base">Lvl {p.level || 1}</div>
